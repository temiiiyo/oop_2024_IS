import sys
from PyQt6.QtWidgets import QApplication, QMainWindow, QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QMessageBox, QTableWidget, QTableWidgetItem, QFileDialog, QComboBox
...
class UserWindow(QMainWindow):
    def __init__(self, db_manager):
        ...
        self.table.setGeometry(10, 10, 580, 300)
    ...
    def load_data(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(currency_share):
                self.table_currency_share.setItem(row_idx, 0, QTableWidgetItem(row[0]))
                self.table_currency_share.setItem(row_idx, 1, QTableWidgetItem(f"{row[1]:.2f}"))
                self.table_currency_share.setItem(row_idx, 2, QTableWidgetItem(f"{row[2] * 100:.2f}%"))
        except DatabaseError as e:
            QMessageBox.critical(self, "Ошибка базы данных", str(e))
...
class AddTransactionDialog(QDialog):
    ...
    def add_transaction(self):
        ...
        try:
            client_id = self.combo_client.currentData()  # Получение ID клиента
            currency_id = self.combo_currency.currentData()  # Получение ID валюты
